...
function coachHomeHtml(){
 ...
 if(!cp||!p){const recs=coachRecommendations(coachGoal(),3),r=recs[0];if(!r)return `${body}<div class="card empty">${esc(tr("residual.r8dcf0f9a"))}</div>`;return `${body}<div class="section">${esc(tr("residual.r0ae2028e"))}</div><div class="card coach-primary"><div class="coach-kicker">${esc(tr("residual.r795da869"))}</div><div class="coach-title">建議先從：${esc(r.p.nameZh)}</div><div class="tagrow" style="margin-top:8px"><span class="tag">長期目標：${esc(coachGoalLabel())}</span><span class="tag">${r.p.daysPerWeek} 日/週</span><span class="tag">約 ${r.p.duration} 分</span><span class="tag">${coachMatchLabel(r.score)}<span class="ui-advanced-only"> ${r.score}/100</span></span></div><div class="coach-reasons">${r.reasons.map(x=>`<div class="coach-reason">${esc(x)}</div>`).join('')}</div>${recommendationBreakdownHtml(r)}<div class="actions" style="margin-top:11px"><button class="btn primary" data-plan-adopt="${esc(r.p.id)}">採用 ${esc(tr('finalUi.weekCount',{count:n(data.settings.blockWeeks)||6}))}計畫</button><button class="btn ghost" id="homeReevaluatePlan">${esc(tr("residual.rac0fcbd0"))}</button></div></div><div class="section">${esc(tr("residual.r715b1c13"))}</div><div class="card empty">${esc(tr("residual.r0c231e77"))}</div>`}
...
function renderHome(){
 ...
 $('#weeklyMuscles').innerHTML=MUSCLES.filter(m=>m!=='有氧'&&m!=='其他').map(m=>{const goal=n(goals[m])||0,val=n(load[m]),pct=goal?Math.min(100,val/goal*100):0;return `<div class="barline"><div class="topline"><b>${m}</b><span>${esc(tr('finalUi.groupsValue',{value:`${val} / ${goal||'—'}`}))}</span></div><div class="progress"><i style="width:${pct}%"></i></div></div>`}).join('');
 ...
 const goalHtml=(data.strengthGoals||[]).map(g=>{const ex=getExercise(g.exerciseId),best=bestSetForExercise(g.exerciseId),cur=best?best.weight:0,pct=g.weight?Math.min(100,cur/g.weight*100):0;return `<div class="record"><div class="record-head"><div><b>目標：${esc(ex?.name||'動作')}</b><div class="record-meta">目前最佳重量 ${fmtWeight(cur)} · ${esc(tr('finalUi.target',{weight:fmtWeight(g.weight),reps:g.reps}))}</div></div><span>${Math.round(pct)}%</span></div><div class="progress"><i style="width:${pct}%"></i></div></div>`}).join('');if(goalHtml)$('#recentProgress').insertAdjacentHTML('beforeend',goalHtml);
...
function showSystemProgramDetail(id){
 ...
 openModal(p.nameZh,`<div class="card"><div class="sys-en">${esc(p.nameEn)}</div><div class="tagrow" style="margin-top:9px"><span class="tag">${esc(p.level)}</span><span class="tag">${p.daysPerWeek} 日/週</span><span class="tag">${p.duration} 分</span><span class="tag">${esc(p.goal)}</span><span class="tag">${esc(p.equipmentMode)}</span></div><div class="sys-desc">${esc(p.descZh)}</div><div class="sys-desc">${esc(p.descEn)}</div>${p.postureSupport?`<div class="warnbox" style="margin-top:10px"><b>${esc(tr('finalUi.supportUse'))} ${infoButton('posture_support')}</b><div class="small" style="margin-top:4px">${esc(p.supportNote||tr('finalUi.supportFallback'))}</div></div>`:''}<div class="hr"></div><div class="small"><b>${esc(tr("residual.r28102a2d"))}</b>${esc(GLOSSARY[progId]?.zh||p.progression)} ${infoButton(progId)}　<b>${esc(tr("residual.rc1960c98"))}</b>${esc(p.rir)} ${infoButton('rir')}</div><div class="small" style="margin-top:7px"><b>${esc(tr("residual.r63314331"))}</b>${esc(programSetSummary(p))}</div></div>
...
function editTemplate(id=''){
 ...
 const showEditor=()=>{
   ...
   openModal(id?tr('modal.editTemplate'):tr('modal.newTemplate'),`<div class="field"><label>${esc(tr('trainingUi.workoutName'))}</label><input id="tplName" value="${esc(working.name)}"></div><div id="tplItems"></div><div class="actions"><button class="btn ghost" id="tplAdd">${esc(tr('trainingUi.addExercise'))}</button><button class="btn primary" id="tplSave">${esc(tr("residual.r2f106cd5"))}</button></div>`,()=>{
     ...
     $('#tplSave').onclick=()=>{working.name=$('#tplName').value.trim()||'未命名課表';if(id){const i=data.templates.findIndex(x=>x.id===id);if(i>=0)data.templates[i]=working}else data.templates.push(working);save(id?tr('finalUi.saveEditTemplate'):tr('finalUi.saveAddTemplate'),true);closeModal();toast(tr('feedback.templateSaved'))}
   })
 };
 showEditor()
}
