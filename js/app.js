...
function coachHomeHtml(){
 ...
 if(!cp||!p){const recs=coachRecommendations(coachGoal(),3),r=recs[0];if(!r)return `${body}<div class="card empty">${esc(tr("residual.r8dcf0f9a"))}</div>`;return `${body}<div class="section">${esc(tr("residual.r0ae2028e"))}</div><div class="card coach-primary"><div class="coach-kicker">${esc(tr("residual.r795da869"))}</div><div class="coach-title">建議先從：${esc(r.p.nameZh)}</div><div class="tagrow" style="margin-top:8px"><span class="tag">長期目標：${esc(coachGoalLabel())}</span><span class="tag">${r.p.daysPerWeek} 日/週</span><span class="tag">約 ${r.p.duration} 分</span><span class="tag">${coachMatchLabel(r.score)}<span class="ui-advanced-only"> ${r.score}/100</span></span></div><div class="coach-reasons">${r.reasons.map(x=>`<div class="coach-reason">${esc(x)}</div>`).join('')}</div>${recommendationBreakdownHtml(r)}<div class="actions" style="margin-top:11px"><button class="btn primary" data-plan-adopt="${esc(r.p.id)}">採用 ${n(data.settings.blockWeeks)||6} 週計畫</button><button class="btn ghost" id="homeReevaluatePlan">${esc(tr("residual.rac0fcbd0"))}</button></div></div><div class="section">${esc(tr("residual.r715b1c13"))}</div><div class="card empty">${esc(tr("residual.r0c231e77"))}</div>`}
 ...
 return `${body}<div class="section">${esc(tr("residual.r0ae2028e"))}</div><div class="card current-plan-card"><div class="record-head"><div><div class="plan-week">第 ${Math.min(week,block)} / ${block} 週${expired?' · 已到重新評估時間':''}</div><div class="current-plan-title">${esc(p.nameZh)}</div><div class="record-meta">長期目標：${esc(COACH_GOALS[cp.goal]?.label||coachGoalLabel())} · ${p.daysPerWeek} 日/週 · 約 ${p.duration} 分</div></div><span class="tag">${coachMatchLabel(r.score)}<span class="ui-advanced-only"> ${r.score}/100</span></span></div><div class="plan-progress"><i style="width:${progress}%"></i></div><div class="actions" style="margin-top:10px"><button class="btn small ghost" id="homeReevaluatePlan">${esc(tr("residual.rabcd84e7"))}</button><button class="btn small ghost" data-coach-preview="${esc(p.id)}">${esc(tr("residual.r73d10828"))}</button><button class="btn small danger" id="homeEndPlan">${esc(tr("residual.rf68c4539"))}</button></div></div><div class="section">${esc(tr("residual.r734e7187"))}</div><div class="card coach-primary"><div class="coach-kicker">${esc(TODAY_ADJUSTMENTS[built.adjustment.mode].label)}${(todayBodyStatus().entries||[]).length?' · 已納入今日身體狀況':''}</div><div class="coach-title">${esc(day.nameZh)}｜${esc(day.dayMeta?.dayTitle||p.nameZh)}</div><div class="tagrow" style="margin-top:8px"><span class="tag">${built.items.length} 個動作</span><span class="tag">原計畫約 ${p.duration} 分</span>${priorityMuscles().length?`<span class="tag">優先：${esc(priorityMuscles().join('、'))}</span>`:''}</div>${coachExerciseListHtml(built.items,p)}<div class="actions" style="margin-top:11px"><button class="btn primary" id="homeCoachStart">${active?'繼續目前訓練':'開始今天訓練'}</button></div></div><div class="section">${esc(tr("residual.r1a342c1b"))}</div><div class="card">${todayAdjustHtml()}<div class="small" style="margin-top:9px">${esc(tr("residual.r008ded82"))}</div></div>`
}
function openPlanReevaluation(){const rs=coachRecommendations(coachGoal(),5);openModal(tr('modal.reevaluatePlan'),`<div class="card"><b>${esc(tr("residual.r4191493c"))}</b><div class="small" style="margin-top:5px">${esc(coachGoalLabel())} · ${n(data.settings.weeklySessions)||3} 日/週 · ${n(data.settings.sessionMinutes)||60} 分 · ${n(data.settings.blockWeeks)||6} 週${priorityMuscles().length?` · 優先 ${esc(priorityMuscles().join('、'))}`:''}</div><div class="small" style="margin-top:5px">${esc(tr("residual.r871527c1"))}</div></div>${rs.map((r,i)=>`<div class="card"><div class="record-head"><div><div class="coach-match">${i+1}. ${coachMatchLabel(r.score)}<span class="ui-advanced-only"> · ${r.score}/100</span></div><div class="coach-alt-title">${esc(r.p.nameZh)}</div><div class="coach-alt-meta">${r.p.daysPerWeek} 日/週 · 約 ${r.p.duration} 分 · ${esc(r.p.goal)}</div></div></div><div class="coach-reasons">${r.reasons.map(x=>`<div class="coach-reason">${esc(x)}</div>`).join('')}</div>${recommendationBreakdownHtml(r)}<div class="actions" style="margin-top:8px"><button class="btn small ghost" data-coach-preview="${esc(r.p.id)}">${esc(tr("residual.r9caf61f6"))}</button><button class="btn small primary" data-plan-adopt="${esc(r.p.id)}">${esc(tr("residual.rd692f32a"))}</button></div></div>`).join('')}`,()=>{bindCoachUI($('#modalBody'))})}
...
function setRowHtml(e,s,ei,si){
 ...
 if(e.type==='unilateral')return `<div class="setrow unilateral ${kcls} ${done}"><div class="setnum">${si+1}</div><label class="set-field"><span class="set-field-label">${esc(tr('trainingUi.leftWeight',{unit}))}</span><input data-set="leftWeight" data-e="${ei}" data-s="${si}" type="number" step=".1" value="${fmtWeightNumber(s.leftWeight,unit)}" placeholder="左${unit}"></label><label class="set-field"><span class="set-field-label">${esc(tr('trainingUi.leftReps'))}</span><input data-set="leftReps" data-e="${ei}" data-s="${si}" type="number" value="${n(s.leftReps)}" placeholder="${esc(tr("residual.r79126c28"))}"></label><label class="set-field"><span class="set-field-label">${esc(tr('trainingUi.rightWeight',{unit}))}</span><input data-set="rightWeight" data-e="${ei}" data-s="${si}" type="number" step=".1" value="${fmtWeightNumber(s.rightWeight,unit)}" placeholder="右${unit}"></label>${intensitySelect(s,ei,si)}<button class="btn icon danger delcell" data-delset="${ei},${si}">−</button><div class="small" style="grid-column:2/-1"><label class="set-field" style="width:110px;display:inline-grid"><span class="set-field-label">${esc(tr('trainingUi.rightReps'))}</span><input data-set="rightReps" data-e="${ei}" data-s="${si}" type="number" value="${n(s.rightReps)}"></label> <span class="weight-unit-suffix">${tr('trainingUi.weightInput',{unit})}</span></div></div>`;
...
function openReplaceModal(idx){
 ...
 openModal(tr('modal.replaceExercise'),list.map(x=>`<button class="btn ghost" data-rp="${x.id}" style="width:100%;margin-bottom:7px;text-align:left">${esc(x.name)}<div class="small">${esc(x.muscle)}</div></button>`).join('')||'<div class="empty">${esc(tr("residual.re7cda89f"))}</div>',()=>$$('[data-rp]').forEach(b=>b.onclick=()=>{const replacement=makeSessionExercise(getExercise(b.dataset.rp),data.activeWorkout.date);data.activeWorkout.exercises[idx]=replacement;saveActiveOnly(true);closeModal()}))
}
...
function showSystemProgramDetail(id){
 ...
 openModal(p.nameZh,`<div class="card"><div class="sys-en">${esc(p.nameEn)}</div><div class="tagrow" style="margin-top:9px"><span class="tag">${esc(p.level)}</span><span class="tag">${p.daysPerWeek} 日/週</span><span class="tag">${p.duration} 分</span><span class="tag">${esc(p.goal)}</span><span class="tag">${esc(p.equipmentMode)}</span></div><div class="sys-desc">${esc(p.descZh)}</div><div class="sys-desc">${esc(p.descEn)}</div>${p.postureSupport?`<div class="warnbox" style="margin-top:10px"><b>使用說明 ${infoButton('posture_support')}</b><div class="small" style="margin-top:4px">${esc(p.supportNote||'此課表為健身補強，不是醫療診斷或治療。')}</div></div>`:''}<div class="hr"></div><div class="small"><b>${esc(tr("residual.r28102a2d"))}</b>${esc(GLOSSARY[progId]?.zh||p.progression)} ${infoButton(progId)}　<b>${esc(tr("residual.rc1960c98"))}</b>${esc(p.rir)} ${infoButton('rir')}</div><div class="small" style="margin-top:7px"><b>${esc(tr("residual.r63314331"))}</b>${esc(programSetSummary(p))}</div></div>
 <div class="program-days">${days}</div><div class="actions" style="margin-top:12px"><button class="btn primary" id="programSetPlan">設為目前 ${n(data.settings.blockWeeks)||6} 週計畫</button><button class="btn ghost" id="programImportNow">${esc(tr("residual.r08c92473"))}</button><button class="btn good" id="programStartNow">${esc(tr("residual.r68a96e5e"))}</button></div>`,()=>{
...
function renderMachineLookupResults(query){
 ...
 if(q.length<2){box.innerHTML='<div class="machine-lookup-empty">輸入至少 2 個字元，例如 <b>Lat Pulldown</b>、<b>Inner / Outer Thigh</b>、<b>Pullover</b>${esc(tr("residual.rba70ff76"))}</div>';return}
 const hits=machineLookupCandidates(q);
 if(!hits.length){box.innerHTML='<div class="machine-lookup-empty">目前找不到相符器械。你仍可以建立自訂動作；也可以嘗試只輸入銘牌上的核心英文，例如 <b>Leg Extension</b>${esc(tr("residual.r2d32bcaa"))}</div>';return}
...
function editExerciseLib(id=''){
 ...
 openModal(id?tr('modal.editExercise'):tr('modal.newExercise'),`${lookupHtml}<div class="grid2"><div class="field"><label>${esc(tr("residual.r01e08503"))}</label><input id="elName" value="${esc(e.name)}"></div><div class="field"><label>${esc(tr("residual.r4be1c58f"))}</label><input id="elNameEn" value="${esc(e.nameEn||'')}"></div><div class="field"><label>${esc(tr("residual.r45b03e28"))}</label><input id="elAliases" value="${esc((e.aliases||[]).join(','))}"></div><div class="field"><label>${esc(tr("residual.rcf870e5d"))}</label><select id="elMuscle">${MUSCLES.map(m=>`<option ${e.muscle===m?'selected':''}>${m}</option>`).join('')}</select></div><div class="field"><label>${esc(tr("residual.re4e4a53e"))}</label><select id="elType">${TYPES.map(([v,l])=>`<option value="${v}" ${e.type===v?'selected':''}>${l}</option>`).join('')}</select></div><div class="field"><label>${esc(tr("residual.r147f7ce4"))}</label><input type="number" id="elSets" value="${e.targetSets}"></div><div class="field"><label>${esc(tr("residual.rf2afec20"))}</label><input type="number" id="elMin" value="${e.repMin}"></div><div class="field"><label>${esc(tr("residual.rb8ab8369"))}</label><input type="number" id="elMax" value="${e.repMax}"></div><div class="field"><label>${esc(tr("residual.r4a5eb0dc"))}</label><input type="number" step=".1" id="elInc" value="${e.increment}"></div><div class="field"><label>${esc(tr("residual.rd7d43a1b"))}</label><input type="number" id="elRest" value="${e.rest}"></div><div class="field"><label>${esc(tr("residual.ra373bd05"))}</label><select id="elEq"><option value="">${esc(tr("residual.r6a86e0bc"))}</option><optgroup label="系統器械">${SYSTEM_EQUIPMENT.map(x=>`<option value="${x.id}" ${e.equipmentId===x.id?'selected':''}>${esc(x.nameZh)} / ${esc(x.nameEn)}</option>`).join('')}</optgroup><optgroup label="我的器材">${data.equipment.map(x=>`<option value="${x.id}" ${e.equipmentId===x.id?'selected':''}>${esc(x.name)}</option>`).join('')}</optgroup></select></div>
 <div class="field"><label>動作模式 <button class="info-btn" type="button" data-info="movement_balance">i</button></label><select id="elPattern"><option value="">${esc(tr("residual.r233b6987"))}</option>${Object.keys(STIMULUS_BY_PATTERN).map(p=>`<option value="${p}" ${e.pattern===p?'selected':''}>${esc(patternDisplayName(p))}</option>`).join('')}</select></div></div>
